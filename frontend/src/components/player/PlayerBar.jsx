import { ChevronUp, ListMusic, MoreVertical, Pause, Play, Repeat2, Shuffle, SkipBack, SkipForward, Volume2, VolumeX } from 'lucide-react'
...
export default function PlayerBar() {
  const { currentTrack, isPlaying, togglePlay, next, previous, progress, duration, seek, volume, setVolume, isShuffleEnabled, isRepeatEnabled, toggleShuffle, toggleRepeat } = usePlayer()
  ...
  return (
    <footer className="fixed inset-x-0 bottom-0 z-40 h-19.5 border-t border-white/10 bg-[#202020] px-4 text-white shadow-2xl sm:px-5">
      ...
      <div className="flex h-full items-center gap-4 sm:gap-6">
        ...
        <button className="ml-3 flex min-w-0 flex-1 items-center justify-start gap-4 text-left sm:max-w-155">
        ...
        <div className="ml-auto flex shrink-0 items-center gap-5 mr-10 text-white/80">
          <button aria-label="More options" className="hidden hover:text-white md:block">
          ...
          <button onClick={() => navigate('/keep-listening')} aria-label="Open queue" className="flex gap-2 hover:text-white">
            <ListMusic size={20} />
            <ChevronUp size={19} className="hidden text-white/60 sm:block" />
          </button>
        </div>
      </div>
    </footer>
